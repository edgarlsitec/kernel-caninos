...
BOARD_CONFIG_DIR=$(CONFIG_DIR)/$(IC_NAME)/boards/$(BOARD_NAME)
...
linux: $(K_BLD_CONFIG)
	...
	$(Q)$(MAKE) -C $(KERNEL_SRC) CROSS_COMPILE=$(CROSS_COMPILE) ARCH=$(ARCH) O=$(KERNEL_OUT_DIR) CONFIG_DEBUG_SECTION_MISMATCH=y -j$(CPUS) uImage modules LOADADDR=0x00008000
...
bootloader:
	...
	$(Q)cd $(TOOLS_DIR)/utils && ./bootloader_pack $(CONFIG_DIR)/$(IC_NAME)/bootloader/bootloader.bin $(BOARD_CONFIG_DIR)/bootloader.ini $(BOOTLOAD_DIR)/bootloader.bin
