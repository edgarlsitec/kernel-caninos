...
struct atc260x_regulator_dev {
	...
	uint suspend_save_state; 
	uint suspend_save_selector;
};
		...
		const struct atc260x_regu_hwinfo *hwinfo, uint band, uint index)
{
	...
	if (band == 0) {
		...
		if(0)
		{
			...
			return sc_revd_buggy_volt_tbl[index] * 1000U;
		}
	} else {
...
static const struct atc260x_regu_hwinfo sc_atc2609a_ldo_hwinfo_tbl[] = {
	...
	{
		.nonlinear_lookup = _atc2609a_ldo6_volt_lookup,
		.volt_bands = {
			{
				.volt_max_uv = 2200000,
				.volt_min_uv =  850000,
...
static int _atc260x_regulator_get_vchng_stable_time(struct regulator_dev *rdev, unsigned int old_selector, unsigned int new_selector)
{
	...
	return(new_volt > old_volt) ? atc260x_regu->regu_hwinfo->stable_time_chng_volt : 0;
}
...
static int _atc260x_regulator_get_actual_status(struct regulator_dev *rdev)
{
	...
	ret = _atc260x_regulator_get_hw_status(atc260x_regu, &state_on, &state_bypass, &state_oc, &state_ov, &state_uv);
...
static int _atc260x_regulator_set_voltage_sel(struct regulator_dev *rdev, unsigned selector)
{
	...
	if(reg_mask)
	{
		//ret = atc260x_reg_setbits(atc260x_regu->atc260x,hwinfo->vsel_reg_addr, reg_mask, (selector << __ffs(reg_mask)) & reg_mask);
	} 
	else
	{
...
static int _atc260x_regulator_enable_inner(struct regulator_dev *rdev, uint enable)
{
	...
	if(reg_mask)
	{
		//ret = atc260x_reg_setbits(atc260x_regu->atc260x, hwinfo->ctrl_reg_addr, reg_mask, (enable ? reg_mask : 0));
	}
	else
	{
		ret = enable ? 0 : -ENXIO;
...
static int _atc260x_regulator_list_voltage(struct regulator_dev *rdev, unsigned selector)
{
	...
	{
		ret = (hwinfo->nonlinear_lookup)(atc260x_regu, hwinfo, atc260x_regu->selected_volt_band, selector);
	...
	else
	{
		...
		else
		{
			...
			{
				if(hwinfo->volt_bands[i].volt_max_uv == 0) {
					break;
				}
...
static int _atc260x_chk_constraints_vrang(struct regulation_constraints *cs, uint v_min, uint v_max, uint correct)
{
	uint ret = 1;
...
static int _atc260x_regulator_set_volt_band(struct atc260x_regulator_dev *atc260x_regu, uint sel_band)
{
	...
	//ret = atc260x_reg_setbits(atc260x, hwinfo->vsel_band_reg_addr, hwinfo->vsel_band_bm, (sel_band << __ffs(hwinfo->vsel_band_bm)) & hwinfo->vsel_band_bm);
		...
		struct regulator_init_data  *init_data)
{
	...
	{
		_atc260x_chk_constraints_vrang(&init_data->constraints, hwinfo->volt_bands[0].volt_min_uv, hwinfo->volt_bands[0].volt_max_uv, true);

		linear_volt_band = &(hwinfo->volt_bands[0]);

		selected_volt_band = 0;

		total_n_volt = hwinfo->volt_bands[0].vsel_end - hwinfo->volt_bands[0].vsel_start + 1U;
	}
	else
	{
		if(hwinfo->volt_band_exclusive == 0)
		{
			_atc260x_chk_constraints_vrang(&init_data->constraints, hwinfo->volt_bands[0].volt_min_uv, hwinfo->volt_bands[1].volt_max_uv, true);

			linear_volt_band = NULL;
			selected_volt_band = 0;
			total_n_volt = hwinfo->volt_bands[1].vsel_end - hwinfo->volt_bands[0].vsel_start + 1U;
		}
		else
		{
			if(_atc260x_chk_constraints_vrang(&init_data->constraints, hwinfo->volt_bands[0].volt_min_uv, hwinfo->volt_bands[0].volt_max_uv, false))
			{
				linear_volt_band = &(hwinfo->volt_bands[0]);
				selected_volt_band = 0;
				total_n_volt = hwinfo->volt_bands[0].vsel_end - hwinfo->volt_bands[0].vsel_start + 1U;
			}
			else if(_atc260x_chk_constraints_vrang(&init_data->constraints, hwinfo->volt_bands[1].volt_min_uv, hwinfo->volt_bands[1].volt_max_uv, false))
			{
				linear_volt_band = &(hwinfo->volt_bands[1]);
				selected_volt_band = 1;
				total_n_volt = hwinfo->volt_bands[1].vsel_end - hwinfo->volt_bands[1].vsel_start + 1U;
			}
			else
			{
...
static void _atc260x_dump_regulator_info(struct atc260x_regulator_dev *atc260x_regu, struct regulator_init_data  *init_data)
{
#if CONFIG_ATC260x_REGU_FULL_DBG
	uint i;
	const struct atc260x_regu_hwinfo *hwinfo;
...
static int atc260x_regulator_print_suspend_state(struct atc260x_regulator_dev *atc260x_regu, const char *prompt)
{
	u8 state_on, state_bypass;
	u8 state_oc, state_ov, state_uv;
	int ret;
...
static int atc260x_regulator_probe(struct platform_device *pdev)
{
	...
	atc260x_regu = devm_kzalloc(&pdev->dev, sizeof(struct atc260x_regulator_dev), GFP_KERNEL);
	...
	ret = _atc260x_init_regulator_desc(atc260x_regu, &(atc260x_regu->regulator_desc), &config, init_data);
	...
	atc260x_regu->regulator = regulator_register(&(atc260x_regu->regulator_desc), &config);
...
{
	atc260x_regulator_print_suspend_state(platform_get_drvdata(pdev), "before shutdown");
}
...
static int atc260x_regulator_resume_early(struct device *dev)
{
	...
	{
		ret = _atc260x_regulator_set_voltage_sel(atc260x_regu->regulator, atc260x_regu->suspend_save_selector);
